...
internal val LiquidShader = """
  ...
  half4 main(float2 fragCoord) {
    ...
    float glassMask = saturate(-sdf / safeSharp * 32.0);
    float transition = smoothstep(0.0, 1.0, glassMask);
    ...
    float2 lens = m2 * sin(pow(saturate(-sdf / lensRefraction), lensCurvature) * HALF_PI) + 0.5;
